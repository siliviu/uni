...
void testAdaugaSiCauta(Relatie less) {
	DO d = DO(less);
	int cMin = 100;
	int cMax = 200;
...
void testStergeCauta(Relatie less) {
	DO d = DO(less);
	int cMin = 10;
	int cMax = 20;
	populeazaDOVidIdentic(d, cMin, cMax);
...
void testIterator(Relatie less) {
	DO d = DO(less);
	Iterator it = d.iterator();
	assert(!it.valid());
	it.prim();
...
void testCerinta() {
	...
	dictOrd.adauga(9, 19);
	dictOrd.adauga(7, 17);
	dictOrd.adauga(16, 26);
	dictOrd.adauga(18, 28);
	dictOrd.adauga(14, 24);
	dictOrd.adauga(11, 21);
	dictOrd.adauga(13, 23);
	assert(dictOrd.diferentaValoareMaxMin() == 15);
}
