...
public class UserController implements IUserObserver {
	...
	public TableColumn<Game, String> column4;
...
//		}
		button.setOnAction(e -> {
			...
			try {
				MoveReply moveReply = service.move(move);
				Alert a = new Alert(Alert.AlertType.INFORMATION);
				a.setContentText(moveReply.isCorrect() ? "Word guessed!" : "No word guessed!");
				a.show();
				if (moveReply.isEnded()) {
					Alert alert = new Alert(Alert.AlertType.INFORMATION);
					Game game = moveReply.getGame();
					Configuration configuration = game.getConfiguration();
					alert.setContentText(String.format("Game over:\nPoints: %d\nWords: %s %s %s %s",
							game.getPoints(),
							configuration.getWord1(),
							configuration.getWord2(),
							configuration.getWord3(),
							configuration.getWord4()
					));
					alert.show();
				}
			} catch (Exception ex) {
		...
		letters.setText(game.getConfiguration().getLetters());
		updateTable();
	}
...
//		column4.setCellValueFactory(new PropertyValueFactory<>("start"));
		column4.setCellValueFactory(celldata -> {
			int seconds = celldata.getValue().getStart();
			Instant instant = Instant.ofEpochSecond(seconds);
			ZoneId zoneId = ZoneId.systemDefault(); // Use the system default time zone
			LocalDateTime localDateTime = instant.atZone(zoneId).toLocalDateTime();
			return new SimpleStringProperty(localDateTime.toString());
		});
