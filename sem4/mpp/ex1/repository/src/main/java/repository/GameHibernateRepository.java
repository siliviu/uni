...
public class GameHibernateRepository implements GameRepository {
	...
	public Collection<Game> getLeaderboard() {
		...
		try (Session session = HibernateUtils.getSessionFactory().openSession()) {
			return session.createQuery("from Game where start > 0 order by points desc", Game.class).getResultList();
		}
	}

	@Override
	public Collection<Game> getGames(String user) {
		logger.traceEntry("get leaderboard");
		try (Session session = HibernateUtils.getSessionFactory().openSession()) {
			return session.createQuery("from Game where start > 0 and user=?1", Game.class).setParameter(1, user).getResultList();
		}
	}
